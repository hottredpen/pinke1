 ...
 * 碎片化内容管理
 */
class CmsBlockAdmin extends CmsBaseAdmin{
    ...
    public function block(){
        ...
        $map          = D('Cms/CmsBlock','Datamanager')->replaceMap($map);
        $data_list    = D("Cms/CmsBlock","Datamanager")->getData($p,$page_size,$map,$order);
        $data_num     = D('Cms/CmsBlock','Datamanager')->getNum($map);

        $builder      = D('Common/List','Builder');
        $builder->theme('one')->setMetaTitle('碎片化内容列表')
                ->setSearch(array('title'=>'内容名称'),'',U('admin/cms/block'))
                ->ajax_url(U('Cms/ajaxCmsBlock'))
                ->addTopButton('custom',array('title'=>'新增','href'=>U('Admin/Cms/addCmsBlock')))
                ...
                ->addRightButton('custom',array('title'=>'修改','href'=>U('Cms/editCmsBlock',array('id'=>'__id__'))))
                ->addRightButton('delete_confirm',array('data-action'=>'deleteCmsBlock','data-itemname'=>'区块内容'))
                ->assign_builder()
    ...
    public function addCmsBlock(){
        ...
        $builder->theme('one')->setMetaTitle('新增碎片块')
                ->setPostUrl(U('Admin/Cms/createCmsBlock'))
    ...
    public function editCmsBlock(){
        $id                       = I('id',0,'intval');
        $info                     = D('Cms/CmsBlock','Datamanager')->getInfo($id);
        $builder  = D('Admin/Form','Builder');
        $builder->theme('one')->setMetaTitle('修改碎片块')
                ->setPostUrl(U('Admin/Cms/saveCmsBlock'))
                ->setPostBackUrl(U('Cms/block'))
                ->addFormItem('category_id', 'select', '分类','',select_list_as_tree('cms_block_category', 'title',array(), 'id','id asc'))
                ->addFormItem('title', 'text', '标题')
                ->addFormItem('list_p_data', 'textarea', '列表显示数据')
