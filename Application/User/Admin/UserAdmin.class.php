...
class UserAdmin extends UserBaseAdmin {
    ...
    public function index() {
        ...
        $builder->theme('one')->setMetaTitle('用户列表')
                ...
                ->addTopButton('layer',array('data-action'=>'addform_User','data-width'=>"800px",'data-height'=>'630px','data-title'=>'新增-用户'))
                ...
                ->addRightButton('layer',array('data-action'=>'editform_User','data-width'=>"800px",'data-height'=>'630px','data-title'=>'编辑-用户信息')) 
                ->assign_builder()
                ->admindisplay('Common@builder:ListBuilder');
    }

    public function addform_User(){
        $info['sex'] = 1; // 默认值
        $builder  = D('Admin/Form','Builder');
        $builder->theme('one')->setMetaTitle('新增用户')
                ->setPostUrl(U('User/addUser'))
