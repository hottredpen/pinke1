...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    $url   = cpk_replace_url($url,$domain);
    return $url;
}
/**
 * 替换url
...
function cpk_replace_url($url,$domain){
...
function cpk_adddomain_url($url){
    $config_array = C('REPLACE_CPK_DOMAIN');
    foreach ($config_array as $key => $value) {
 ...
 * @return void
 * 为防止pjax时，多次跳转增加了以下 hottredpen 2017.3.3  
 *  && I("_pjax","","trim") == ""
 */
